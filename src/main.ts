...
function runApp(): void {
  ...
  function applyState(state: RoomState): void {
    ...
    if (state.phase === "lobby") {
      ...
      if (dom.shareRoundBtn) {
        dom.shareRoundBtn.hidden = !state.lastRoundId;
      }
  ...
  if (dom.shareRoundBtn) {
    dom.shareRoundBtn.addEventListener("click", async () => {
      const roundId = currentState?.lastRoundId;
      if (!roundId) {
      ...
      const shareText = buildShareText(currentState, shareUrl);

      // Native share sheet (mobile, Safari, Edge). Best UX on phones
      // where WhatsApp / Messages / etc. show up inline. We deliberately
      // pass only `text` — WhatsApp concatenates `title` into the body
      // which produces a visible duplicate of the room code.
      if (navigator.share) {
        try {
          ...
          // don't fall through to clipboard (they already picked "no").
          if ((err as Error).name === "AbortError") return;
          /* any other failure falls through to clipboard */
        }
      }

      // Clipboard fallback. Requires a secure context — on plain http
      // origins (not localhost) this is undefined.
      ...
      // Last-resort fallback: a modal prompt. User can copy manually.
